  ...
  return (
    <div className="min-h-screen flex">
          ...
          <div className="mt-10 space-y-3 text-left max-w-xs mx-auto">
            {[{icon:'🧾',label:'Fast Billing'},{icon:'💳',label:'Multi-Payment Support'},{icon:'📦',label:'Live Inventory View'}].map(f => (
              <div key={f.label} className="flex items-center gap-3 px-4 py-2 rounded-lg" style={{ background:'rgba(255,255,255,0.05)' }}>
                <span>{f.icon}</span><span className="text-sm" style={{ color:'#94a3b8' }}>{f.label}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="w-[420px] bg-white flex flex-col justify-center px-12">
        ...
        {error && <div className="mb-4 px-4 py-3 rounded-lg text-sm font-medium" style={{ background:'#fee2e2', color:'#991b1b', border:'1px solid #fecaca' }}>⚠ {error}</div>}
        <div className="mb-4">
          <label className="block text-sm font-medium text-[#0f172a] mb-1.5">Email Address</label>
          <input type="email" value={email} onChange={e => { setEmail(e.target.value); setError(''); }} placeholder="cashier123@gmail.com"
            className="w-full px-4 py-2.5 text-sm bg-[#f8fafc] border border-[#e2e8f0] rounded-lg outline-none text-[#0f172a] transition-all"
        ...
        <div className="text-center mb-3"><span className="text-[10px] text-[#94a3b8] font-mono tracking-widest uppercase">2FA Verification</span></div>
