// src/pages/cashier/Profile.jsx
import { useState } from 'react';
import CashierLayout from '../../layouts/CashierLayout';
import { Toggle } from '../../components/common';

const cashierUser = { name:'Kasim Rijal', initials:'KR', role:'Cashier', email:'kasim@store.np', phone:'+977-9841-000123', store:'KTM-001 — Kathmandu', shift:'#0842' };
const shiftHistory = [
    ...
    <CashierLayout>
      <div className="p-8 max-w-[900px]">
        ...
        <div className="rounded-xl p-6 mb-5 flex items-center gap-5" style={{background:'#0f172a'}}>
          ...
          <div>
            <h2 className="text-xl font-bold text-white">{cashierUser.name}</h2>
            <p className="text-sm mt-0.5" style={{color:'#94a3b8'}}>{cashierUser.role} · {cashierUser.store}</p>
            <div className="flex items-center gap-3 mt-2">
              <span className="text-xs font-mono px-2.5 py-0.5 rounded-full" style={{background:'rgba(255,255,255,0.1)',color:'#cbd5e1'}}>Shift {cashierUser.shift}</span>
              <div className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-[#22c55e]" /><span className="text-xs text-[#94a3b8]">On Shift</span></div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 mb-4">
          ...
          <div className="bg-white rounded-xl border p-5" style={{borderColor:'#e2e8f0'}}>
            ...
            <div className="space-y-3">
              {[{label:'Full Name',value:cashierUser.name},{label:'Email',value:cashierUser.email},{label:'Phone',value:cashierUser.phone},{label:'Store',value:cashierUser.store}].map(f => (
                <div key={f.label}>
          ...
          <div className="bg-white rounded-xl border p-5" style={{borderColor:'#e2e8f0'}}>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div><p className="text-sm font-medium text-[#0f172a]">Receipt Sound</p><p className="text-xs text-[#94a3b8]">Play sound on transaction complete</p></div>
                <Toggle checked={receiptSound} onChange={setReceiptSound} />
              </div>
              <div className="flex items-center justify-between">
                <div><p className="text-sm font-medium text-[#0f172a]">Low Stock Alerts</p><p className="text-xs text-[#94a3b8]">Show alerts for low inventory</p></div>
                <Toggle checked={notifAlerts} onChange={setNotifAlerts} />
              </div>
            </div>
            <div className="mt-5 pt-4 border-t" style={{borderColor:'#e2e8f0'}}>
              <p className="text-xs text-[#94a3b8] mb-3">Change PIN</p>
              <input type="password" placeholder="Enter new 4-digit PIN" className="input-field text-sm mb-2" maxLength={4} />
              <button className="btn-primary text-xs px-4 py-2">Update PIN</button>
            </div>
          </div>
        </div>
            ...
            <tbody>
              {shiftHistory.map(s => (
                <tr key={s.id}>
                  ...
                  <td>{s.status ? <span className="text-xs px-2 py-0.5 rounded bg-[#dcfce7] text-[#15803d] font-medium">{s.status}</span> : <span className="text-xs text-[#94a3b8]">Closed</span>}</td>
                </tr>
              ))}
            </tbody>
          </table>
